...
# conv_distiller = nn.ModuleList ([])
#
# for f in out_feat[:-1]:
#     print('hello', f.shape)
#     TempDistiller (in_channels=f.size (1), in_resolution=f.size (2)).cuda ()
#     print("end")
conv_distiller = nn.ModuleList (
    [TempDistiller (in_channels=f.size (1), in_resolution=f.size (2), final_dim=final_dim).cuda () for f in
     out_feat[:-1]])
conv_distiller.append (FinalClassifier (out_feat[-1].size (1), final_dim=final_dim, num_classes=n_cls).cuda())
from torchsummary import summary

for conv in conv_distiller:
    num_param = 0
    for name, w in conv.named_parameters():
        # print(name, w.shape)
        num_param += np.prod(w.shape)
    print(num_param)

# for conv, out_ in zip (conv_distiller, out_feat):
#     print(out_.shape, final_dim)
#     summary(conv, out_[0].shape)
...
for epoch in range (1, total_epoch + 1):
    ...
    for idx, data in enumerate (train_loader):
        ...
        for idx_, (conv, fs, ft) in enumerate (zip (trainable_models.conv_distiller[::-1], feat_s[::-1], feat_t[::-1])):
            ...
            loss_1 += fit_alpha * (kd_loss (conv_logit_s, (logit_t.detach () + prev_activation) / 2) * (kd_T ** 2))

            # inter-layer distance
            #loss_2 += 1e-2 * criterion_MSE (conv_logit_s, prev_activation)
        ...
        # loss_simple = ((balance * F.kl_div (F.log_softmax (simple_out_t / kd_T, dim=1),
        ...
        loss_simple = balance * kd_loss(simple_out_s, (logit_t.detach() + simple_out_t) / 2) * (kd_T ** 2) / logit_s.size (0)

        loss_batch_list.append (loss_simple.item ())
        loss_kd = (kd_T ** 2) * F.kl_div (F.log_softmax (logit_s / kd_T, dim=1),
                                          F.softmax (logit_t.detach () / kd_T, dim=1),
                                          size_average=False) / logit_s.size (0)
    ...
    print (
        'Epoch: {}, [Train]* Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f} Acc_Ensemble@1 {top1_ens.avg:.3f} Acc_Ensemble@5 {top5_ens.avg:.3f} Acc_Ensemble_Sub@1 {top1_ens_sub.avg:.3f} Acc_Ensemble_Sub@5 {top5_ens_sub.avg:.3f}'
        .format (epoch, top1=top1, top5=top5, top1_ens=top1_ensemble, top5_ens=top5_ensemble,
                 top1_ens_sub=top1_ensemble_sub, top5_ens_sub=top5_ensemble_sub))

    loss_list = np.array (loss_list)
    avg_loss_list = np.sum (loss_list, axis=0) / count

    print (
        "ConvDistiller: %.4f, InterFeature: %.4f, Conv_FM: %.4f, Feature_CE: %.4f Loss(simple): %.4f CE: %.4f FM: %.4f" %
    ...
    print (
        ' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f} Acc_Ensemble@1 {top1_ens.avg:.3f} Acc_Ensemble@5 {top5_ens.avg:.3f} Acc_Sub_Ensemble@1 {top1_ens_sub.avg:.3f} Acc_Sub_Ensemble@5 {top5_ens_sub.avg:.3f} Acc_Sub2_Ensemble@1 {top1_ens_sub2.avg:.3f} Acc_Sub2_Ensemble@5 {top5_ens_sub2.avg:.3f} Best_accuracy %.4f Best_Ensemble_accuarcy %.4f'
        .format (top1=val_top1, top5=val_top5, top1_ens=val_top1_ensemble, top5_ens=val_top5_ensemble,
                 top1_ens_sub=val_top1_ensemble_sub_1, top5_ens_sub=val_top5_ensemble_sub_1,
                 top1_ens_sub2=val_top1_ensemble_sub_2, top5_ens_sub2=val_top5_ensemble_sub_2) % (
