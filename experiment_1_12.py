...
class TempDistiller(nn.Module):
    def __init__(self, in_channels, in_resolution, final_dim=256, num_classes=100):
...
#             print(temp)
    ...
    def forward(self, in_feat):
        ...
        return out_fc, feature, torch.mean(feature, axis=0), torch.var(feature, axis=1, unbiased=False)


## added in exp1_8
class FinalClassifier(nn.Module):
    def __init__(self, in_channels, final_dim=256, num_classes=100):
...
#         self.bn = nn.BatchNorm1d(final_dim, affine=True, track_running_stats=True)
    ...
    def forward(self, x):
        out_fc = self.fc2(x)
        return out_fc, x, torch.mean(x, axis=0), torch.var(x, axis=1, unbiased=False)
...
#         prev_activation = feat_t[-1].detach()
        ...
        for idx_, (conv, fs, ft) in enumerate(zip(trainable_models.conv_distiller[::-1], feat_s[::-1], feat_t[::-1])):
            conv_logit_t, conv_feature_t, running_mean, running_var = conv(ft.detach())
            if idx_ == 0:
                r_mean, r_var = running_mean.clone(), running_var.clone()
            
            conv_logit_s, conv_feature_s, running_mean, running_var = conv(fs)
            ensemble += conv_logit_s # ensemble
            
            
            
            # Feature SP Distillation
            loss_1 += balance * (kd_loss(conv_logit_t, logit_t.detach()) * (kd_T ** 2))
            loss_1 += (1 - balance) * (kd_loss(conv_logit_s, conv_logit_t.detach()) * (kd_T ** 2))
...
#             loss_1 += balance * (kd_loss(conv_logit_s, logit_t.detach()) * (kd_T ** 2))
            ...
            # top-conv-feature
            # direct
            loss_2 += torch.dist(running_mean, r_mean.detach())
            loss_2 += torch.dist(running_var, r_var.detach())
            
            # FitNet MSE (feature teacher - feature student)
            loss_3 += criterion_AB(fs, ft.detach())
    ...
    with torch.no_grad():
        ...
        for idx, (input, target) in enumerate(val_loader):
            ...
            for k, (conv, fs) in enumerate(zip(trainable_models.conv_distiller, val_feat_s)):
                conv_logit_s, conv_feature_s, _, _ = conv(fs)
                ensemble += conv_logit_s # ensemble
