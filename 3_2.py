...
class SepConv (nn.Module):
    def __init__(self, channel_in, channel_out, kernel_size=3, stride=2, padding=1, affine=True, max_channel=256):
        ...
        self.op = nn.Sequential (
            ...
            nn.Conv2d (channel_in * self.cadinality, channel_in * self.cadinality, kernel_size=kernel_size, stride=1,
                       padding=padding, groups=channel_in * self.cadinality, bias=False),
...
for epoch in range (1, total_epoch + 1):
    ...
    for idx, data in enumerate (train_loader):
        ...
        # prev_activation = feat_t[-1].detach ()
        ...
        for idx_, (conv, fs, ft) in enumerate (zip (trainable_models.conv_distiller[::-1], feat_s[::-1], feat_t[::-1])):
            ...
            #loss_2 += 1e-2 * criterion_MSE (conv_feature_s, prev_activation)
            ...
            cc += 1
            #prev_activation = conv_feature_s.detach ()
    ...
    print (
        ' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f} Acc_Ensemble@1 {top1_ens.avg:.3f} Acc_Ensemble@5 {top5_ens.avg:.3f} Acc_Sub_Ensemble@1 {top1_ens_sub.avg:.3f} Acc_Sub_Ensemble@5 {top5_ens_sub.avg:.3f} Best_accuracy %.4f Best_Ensemble_accuarcy %.4f'
        .format (top1=val_top1, top5=val_top5, top1_ens=val_top1_ensemble, top5_ens=val_top5_ensemble,
                 top1_ens_sub=val_top1_ensemble_sub, top5_ens_sub=val_top5_ensemble_sub) % (
    ...
    print ("Ensemble Sub Acc1@ {:.3f}".format (val_top1_ensemble_sub.avg))