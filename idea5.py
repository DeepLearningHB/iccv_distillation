...
for epoch in range(1, total_epoch+1):
    ...
    for idx, data in enumerate(train_loader):
        ...
        loss_simple = (balance * ((F.softmax(simple_out_t / kd_T) - F.softmax(logit_t.detach() / kd_T)) ** 2).mean()) + (1 - balance) * ((F.softmax(simple_out_s / kd_T )- F.softmax(simple_out_t.detach() / kd_T)) ** 2).mean()
        ...
        count += 1
        loss = r * loss_ce + a * loss_origin + b * loss_simple + p * loss_fm
        losses.update(loss)
