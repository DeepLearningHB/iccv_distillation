...
#             similarity_t_list.append(cos(simple_val_out_s, output).mean().item())
    ...
    print(' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f} Acc_Ensemble@1 {top1_ens.avg:.3f} Acc_Ensemble@5 {top5_ens.avg:.3f} Acc_Sub_Ensemble@1 {top1_ens_sub.avg:.3f} Acc_Sub_Ensemble@5 {top5_ens_sub.avg:.3f} Best_accuracy %.4f Best_Ensemble_accuarcy %.4f'
          .format(top1=val_top1, top5=val_top5, top1_ens=val_top1_ensemble, top5_ens=val_top5_ensemble, top1_ens_sub=val_top1_ensemble_sub, top5_ens_sub=val_top5_ensemble_sub) % (best_accuracy, best_ensemble_acc))
    print(" * ", end='')
