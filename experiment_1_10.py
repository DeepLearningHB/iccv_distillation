...
#     conv_distiller.train()
    ...
    for idx, data in enumerate(train_loader):
        ...
        for idx_, (conv, fs, ft) in enumerate(zip(trainable_models.conv_distiller[::-1], feat_s[::-1], feat_t[::-1])):
            ...
            loss_1 += 0.02 * (kd_loss(conv_logit_t, logit_t.detach()) * (kd_T ** 2))
            loss_1 += 0.1 * (kd_loss(conv_logit_s, conv_logit_t.detach()) * (kd_T ** 2))
...
#             loss_1 += balance * (kd_loss(conv_logit_s, logit_t.detach()) * (kd_T ** 2))
            ...
            if idx_ == 0:
                loss_2 += criterion_AB(conv_feature_s, ft)
            else:
                loss_2 += criterion_AB(conv_feature_s, prev_activation)
