...
balance = opt.balance
model_t = model_dict[opt.model_t](num_classes=10)
...
train_loader, val_loader, n_data = get_cifar10_dataloaders(batch_size=train_batch_size,
...
class SepConv(nn.Module):
    def __init__(self, channel_in, channel_out, kernel_size=3, stride=2, padding=1, affine=True, max_channel=MAX_DIM):
        ...
        self.op = nn.Sequential(
            ...
            nn.Conv2d(inter_channel, inter_channel, kernel_size=kernel_size, stride=1, padding=padding,
                      groups=inter_channel, bias=False),
...
class TempDistiller(nn.Module):
    def __init__(self, in_channels, in_resolution, final_dim=64, num_classes=10):
...
conv_distiller = nn.ModuleList(
    [TempDistiller(in_channels=f.size(1), in_resolution=f.size(2), final_dim=final_dim).cuda() for f in out_feat[:-1]])
conv_distiller.append(FinalClassifier(out_feat[-1].size(1), num_classes=n_cls))
MLP = SimpleMLP(out_feat[-1].size(1), 10).cuda()
...
for epoch in range(1, total_epoch + 1):
    ...
    for idx, data in enumerate(train_loader):
        ...
        loss_kd = (kd_T ** 2) * F.kl_div(F.log_softmax(logit_s / kd_T, dim=1),
                                         F.softmax(logit_t.detach() / kd_T, dim=1), size_average=False) / logit_s.size(
            0)
