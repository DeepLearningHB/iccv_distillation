...
class TempDistiller (nn.Module):
    def __init__(self, in_channels, in_resolution, final_dim=32, num_classes=100):
        ...
        while temp > 4:
            self.module_list.append (SepConv (
                channel_in=in_channels,
                channel_out= min(in_channels * 2, max_feat_num)
            ))

            in_channels = min(in_channels * 2, max_feat_num)
            temp /= 2
        ...
        if flag:
            ...
            self.fc = nn.Linear(min(in_channels, max_feat_num), num_classes)
...
conv_distiller = nn.ModuleList (
    [TempDistiller (in_channels=f.size (1), in_resolution=f.size (2), final_dim=final_dim).cuda () for f in
     out_feat[:-1]])
conv_distiller.append (FinalClassifier (out_feat[-1].size(1), num_classes=n_cls).cuda())
from torchsummary import summary
